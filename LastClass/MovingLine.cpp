...
void MovingLine::MouseLButtonDrag(MouseLButton *mouseLButton, Diagram *diagram, Selection *selection, Long  startX, Long startY, Long currentX, Long currentY, CPaintDC *cPaintDC) {
	...
	if (selectedClass->GetAttributePosition() != -1) {
		attributeStartPoint.x = selectedClass->GetAt(selectedClass->GetAttributePosition() - 1)->GetX();
		attributeStartPoint.y = selectedClass->GetAt(selectedClass->GetAttributePosition() - 1)->GetY();
		attributeEndPoint.x = selectedClass->GetAt(selectedClass->GetAttributePosition() - 1)->GetX() + selectedClass->GetAt(selectedClass->GetAttributePosition() - 1)->GetWidth();
		attributeEndPoint.y = selectedClass->GetAt(selectedClass->GetAttributePosition() - 1)->GetY();
		ret = finder.FindLineByArea(attributeStartPoint, attributeEndPoint, rect);
		
		if (ret == true) {
			cPaintDC->MoveTo(attributeStartPoint.x, attributeStartPoint.y + distanceY);
			cPaintDC->LineTo(attributeEndPoint.x, attributeEndPoint.y + distanceY);
		}
	}
	if (selectedClass->GetMethodPosition() != -1 && ret != true) {
		methodStartPoint.x = selectedClass->GetAt(selectedClass->GetMethodPosition() - 1)->GetX();
		methodStartPoint.y = selectedClass->GetAt(selectedClass->GetMethodPosition() - 1)->GetY();
		methodEndPoint.x = selectedClass->GetAt(selectedClass->GetMethodPosition() - 1)->GetX() + selectedClass->GetAt(selectedClass->GetMethodPosition() - 1)->GetWidth();
		methodEndPoint.y = selectedClass->GetAt(selectedClass->GetMethodPosition() - 1)->GetY();
		ret = finder.FindLineByArea(methodStartPoint, methodEndPoint, rect);
		if (ret == true) {
			cPaintDC->MoveTo(methodStartPoint.x, methodStartPoint.y + distanceY);
			cPaintDC->LineTo(methodEndPoint.x, methodEndPoint.y + distanceY);
		}
	}
	if (selectedClass->GetReceptionPosition() != -1 && ret != true) {
		receptionStartPoint.x = selectedClass->GetAt(selectedClass->GetReceptionPosition() - 1)->GetX();
		receptionStartPoint.y = selectedClass->GetAt(selectedClass->GetReceptionPosition() - 1)->GetY();
		receptionEndPoint.x = selectedClass->GetAt(selectedClass->GetReceptionPosition() - 1)->GetX() + selectedClass->GetAt(selectedClass->GetReceptionPosition() - 1)->GetWidth();
		receptionEndPoint.y = selectedClass->GetAt(selectedClass->GetReceptionPosition() - 1)->GetY();
		ret = finder.FindLineByArea(receptionStartPoint, receptionEndPoint, rect);
		if (ret == true) {
			cPaintDC->MoveTo(receptionStartPoint.x, receptionStartPoint.y + distanceY);
			cPaintDC->LineTo(receptionEndPoint.x, receptionEndPoint.y + distanceY);
		}
	}
