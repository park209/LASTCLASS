...
if (int_ptr == IDOK) {
	...
	Long vmax=0;
	Long hmax=0;
	lastClass->classDiagramForm->SetScrollRange(SB_HORZ, min, hmax);
	lastClass->classDiagramForm->SetScrollRange(SB_VERT, min, vmax);
	Long hPos = lastClass->classDiagramForm->GetScrollPos(SB_HORZ);
	Long vPos = lastClass->classDiagramForm->GetScrollPos(SB_VERT);
	scrollMovingObject.MovingObject(lastClass->classDiagramForm->diagram, hPos, vPos);
	...
	scrollMovingObject.MovingObject(lastClass->classDiagramForm->diagram, -hPos, -vPos);
	lastClass->classDiagramForm->SetScrollRange(SB_HORZ, 0, hmax);
	lastClass->classDiagramForm->SetScrollRange(SB_VERT, 0, vmax);
}
