...
void WritingVisitor::Visit(MemoBox *memoBox, CDC *cPaintDc) {
	...
	cPaintDc->DrawText((CString)memoBox->GetContent().c_str(), &rt, DT_EXPANDTABS | DT_TABSTOP | 0x0600);
}
...
void WritingVisitor::Visit(ClassName* className, CDC* cPaintDc) {
	...
	cPaintDc->DrawText((CString)className->GetContent().c_str(), &rt, DT_EXPANDTABS | DT_TABSTOP | 0x0600);
}
void WritingVisitor::Visit(Attribute* attribute, CDC* cPaintDc) {
	...
	cPaintDc->DrawText((CString)attribute->GetContent().c_str(), &rt, DT_EXPANDTABS | DT_TABSTOP | 0x0600);
}
void WritingVisitor::Visit(Method* method, CDC* cPaintDc) {
	...
	cPaintDc->DrawText((CString)method->GetContent().c_str(), &rt, DT_EXPANDTABS | DT_TABSTOP | 0x0600);
}
void WritingVisitor::Visit(Reception* reception, CDC* cPaintDc) {
	...
	cPaintDc->DrawText((CString)reception->GetContent().c_str(), &rt, DT_EXPANDTABS | DT_TABSTOP | 0x0600);
}
...
void WritingVisitor::Visit(Text* text, CDC* cPaintDc) {
	...
	cPaintDc->DrawText((CString)text->MakeText().c_str(), &rt, DT_EXPANDTABS | DT_TABSTOP | 0x0600);
}