...
void EditResizer::ResizeEdit(TextEdit *textEdit, CDC *cdc) {
	...
	if (textEdit->text->MaxWidth(cdc) > rt.right - GabX *3) {				//글너비가 클래스를 넘어가는데
	...
	else if (textEdit->text->MaxWidth(cdc) <= textEdit->figure->GetWidth() - GabX * 3) {		//글너비가 클래스를 안넘어가는데
...
void EditResizer::ResizeClass(TextEdit *textEdit, CDC *cdc) {
	...
	if (textEdit->text->MaxWidth(cdc) > textEdit->GetCriteriaWidth() - GabX * 3/*rt.right - GabX *3*/) {
